import Button from '@/Components/Common/Button'
import { Input } from '@/Components/Forms'
import { useCaseStates } from '@/Components/Hooks/utils'
import BaseModal from '@/Components/Modals/BaseModal'
import { useAddStateMutation, useEditStateMutation } from '@/redux/api/utilsApi'
import React, { useEffect } from 'react'
import { toast } from 'react-toastify'

interface stateType{
    id?:string,
    name:string
}
interface Props{
    handleToggler:()=>void,
    open:boolean,
    oldState?: stateType|null
}

const StateFormModal = ({handleToggler, open, oldState}:Props) => {    
    const [addState, {isLoading}] = useAddStateMutation()
    const [editState, {isLoading:editLoading}] = useEditStateMutation()
    const {
        caseState,
        error,
        onChange,
        setError,
        setCaseState
    } = useCaseStates()
    useEffect(()=>{
        if(!open){
            setCaseState({id:'', name:''})
        }
    }, [open])
    
    useEffect(()=>{
        if(oldState)
            setCaseState(oldState)
    }, [oldState?.id])
    
    const handleState = () =>{
        if(error){
            toast.error('برجاء إدخال بيانات حالة القضية بشكل صحيح')                
            return
        }
        if(caseState.id){
            editState({id:caseState.id, name:caseState.name})
            .unwrap()
            .then(res=>{
                handleToggler()
                setCaseState({id:'', name:''})                
                toast.success(res?.message)        
            }).catch(err=>{
                console.log(err);
                const error = err?.data?.errors
                if(!error)
                    toast.error('حدث خطأ ما برجاء المحاولة لاحقا')        
                else        
                    setError(error)
            })

        }else{
            addState({name:caseState.name})
            .unwrap()
            .then(res=>{
                handleToggler()
                setCaseState({id:'', name:''})
                toast.success(res?.message)
            }).catch(err=>{
                console.log(err);
                const error = err?.data?.errors
                if(!error)
                    toast.error('حدث خطأ ما برجاء المحاولة لاحقا')        
                else        
                    setError(error)
            })
        }
    }
    return (
        <BaseModal
            handleToggler={handleToggler}
            open={open}
        >
            <div className="w-[50vw] my-8">
                <Input
                    label='اسم حالة القضية'
                    labelId='state'
                    type='text'
                    value={caseState.name}
                    onChange={e=>onChange(e, {regex:{value:'^[ء-ي][ء-ي ]{2,}$', message:"اسم حالة القضية يجب ان يحتوي على الاقل من 3 أحرف باللغة العربية"}})}
                    errors={error}
                    placeholder='اسم حالة القضية'
                    required
                />
            </div>
            <div className="grid grid-cols-2 gap-2">
                <Button onClick={handleState} className='bg-primary hover:bg-transparent hover:text-black border-primary text-white' title={'حفظ'} isLoading={isLoading||editLoading} />
                <Button 
                    className='w-full py-2 rounded-lg border border-secondary text-center hover:bg-secondary hover:text-white transition-all'
                    onClick={handleToggler}
                    isLoading={false}
                    title='إلغاء'
                />
        </div>
        </BaseModal>
    )
}

export default StateFormModal
